...
my ($sura, $start_ayah, $end_ayah, $batch, $width, $scale, $help) = (undef, undef, undef, undef, 640, 1.0, 0);
...
die "Minimal parameters are both --sura and --ayah for a single verse, or use \
     --batch to generate images for the entire Qur'an" unless $batch or ($sura and $start_ayah and $end_ayah);

if ($batch) {
    $self->generate_batch;
}
else {
    $self->generate_image($sura, $start_ayah, $end_ayah);
}
...
sub generate_image {
    my ($self, $sura, $start_ayah, $end_ayah) = @_;

    print "Generating image for sura $sura, ayah $start_ayah to $end_ayah...\n";

    my ($page, $text) = $dbh->selectrow_array(
        "select page, text from sura_ayah_page_text where sura = $sura and ayah >= $start_ayah and ayah <= $end_ayah");
    ...
    my ($last_line, $line_count) = $dbh->selectrow_array(
        "select max(line), count(*) from madani_page_text where sura = $sura and ayah >= $start_ayah and ayah <= $end_ayah");
    my $sth = $dbh->prepare(
        "select ayah, line, text from madani_page_text where sura = $sura and ayah >= $start_ayah and ayah <= $end_ayah order by ayah, line asc");
    $sth->execute;
    while (my ($ayah, $line_number, $line_text) = $sth->fetchrow_array) {
    ...
    my $file = $sura ."_". $start_ayah . $end_ayah .".png";
