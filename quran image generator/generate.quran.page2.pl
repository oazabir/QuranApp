...
my ($page, $batch, $width, $scale, $help, $start, $end, $surah) = (undef, undef, undef, 1.0, 0, 0, 0, 0);
...
die "Minimal parameters are --width and --page for a single page, or --width \
     and --batch for the entire Qur'an" unless $width and ($batch or $page or $surah);
...
sub generate_page {
	...
	my $sth = {};
	if ($surah > 0) {
		$filename = sprintf('%03d_%03d_%03d', $surah, $start, $end);
		
		$sth = $dbh->prepare(
			"select m.page, m.line, m.ayah, s.text from madani_page_text m
			inner join sura_ayah_page_text s on s.sura = m.sura and s.ayah = m.ayah where m.sura = $surah and m.ayah >= $start and m.ayah <= $end order by m.ayah, m.line");		
	}
	else {
		$filename = sprintf('%03d', $page);
		
		$sth = $dbh->prepare(
			"select page, line, ayah, text from madani_page_text where page=$page");
	}
	
	$sth->execute;
	while (my ($page, $line, $ayah, $text) = $sth->fetchrow_array) {
		my $page_str = sprintf('%03d', $page);
		$text = $self->_reverse_text($text);
		printf("%d %d %s", $ayah, $line, $text);
		
		$hash->{$ayah.$line} = [$ayah, $text, $page];
		...
		if ($line_width > $longest_line_width) {
			$longest_line = $ayah.$line;
			$longest_line_width = $line_width;
		}
	}
	$sth->finish;

	my ($ayah, $text, $page2) = @{$hash->{$longest_line}};
	...
	for my $line (sort { $a <=> $b } keys %{ $hash }) {
		...
		my $text = @{$hash->{$line}}[1];
		my $page = @{$hash->{$line}}[2];
		$_draw_line->($linecounter, $ayah, $text, $page);
		$linecounter++;
	}

	my $path = "./output/width_$width/";
	eval { `mkdir -p $path` };
	open OUTPUT, ">$path/$filename.png";
	binmode OUTPUT;
